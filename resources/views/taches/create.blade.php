                    ...
                    <div class="p-8 bg-white border-b border-gray-200">
                        ...
                        @if ($errors->any())
                            <div class="mb-6 p-6 bg-red-100 border border-red-400 text-red-700 rounded-lg shadow-md animate-fade-in animate-pulse-subtle">
                                ...
                                <span class="block sm:inline">{{ __('Il y a eu des problèmes avec votre soumission.') }}</span>
                        ...
                        <form action="{{ route('taches.store') }}" method="POST" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                <div>
                                    ...
                                    <input type="text" name="titre" id="titre" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('titre') border-primary-red ring-red-200 @enderror" value="{{ old('titre') }}" placeholder="{{ __('Entrez un titre concis...') }}" required>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="4" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('description') border-primary-red ring-red-200 @enderror" placeholder="{{ __('Décrivez la tâche ou utilisez l\'audio...') }}">{{ old('description') }}</textarea>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <div class="flex flex-wrap items-center space-x-2 mt-1">
                                        <button type="button" id="recordButton" class="btn btn-primary-red px-4 py-2 rounded-lg text-sm font-semibold text-white"><i class="fas fa-microphone mr-1"></i> Démarrer</button>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="duree" id="duree" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('duree') border-primary-red ring-red-200 @enderror" value="{{ old('duree') }}" placeholder="{{ __('Ex: 15 minutes, 2 heures, 1 jour...') }}" required>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="datedebut" id="datedebut" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('datedebut') border-primary-red ring-red-200 @enderror" value="{{ old('datedebut') }}" required>
                                ...
                                <div id="heuredebutContainer" style="display: none;">
                                    ...
                                    <input type="time" name="heuredebut" id="heuredebut" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('heuredebut') border-primary-red ring-red-200 @enderror" value="{{ old('heuredebut') }}">
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('status') border-primary-red ring-red-200 @enderror" required>
                                        ...
                                        {{-- ✅ AJOUT : Annulé visible uniquement aux admins --}}
                                        @if($isAdmin)
                                ...
                                <div>
                                    ...
                                    <select name="date" id="date" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('date') border-primary-red ring-red-200 @enderror" required>
                                        <option value="minute" {{ old('date') == 'minute' ? 'selected' : '' }}>{{ __('Minute') }}</option>
                                        <option value="heure" {{ old('date') == 'heure' ? 'selected' : '' }}>{{ __('Heure') }}</option>
                                ...
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1" for="user_ids">
                                        <i class="fas fa-users mr-2 text-cyan-600"></i> {{ __('Assigné(e) à') }} <span class="text-primary-red text-lg">*</span>
                                    </label>
                                    <div class="border border-gray-300 rounded-lg p-3 grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                                        @foreach ($users as $userItem)
                                            <div class="flex items-center">
                                                <input class="rounded border-gray-300 text-primary-red shadow-sm focus:border-primary-red focus:ring focus:ring-primary-red focus:ring-opacity-50" type="checkbox" name="user_ids[]" value="{{ $userItem->id }}" id="user_{{ $userItem->id }}" {{ in_array($userItem->id, old('user_ids', [])) ? 'checked' : '' }}>
                                                <label class="ml-2 text-sm text-gray-800" for="user_{{ $userItem->id }}">{{ $userItem->name }} ({{ $userItem->email }})</label>
                                            </div>
                                        @endforeach
                                    </div>
                                    @error('user_ids')<p class="text-primary-red text-xs mt-1">{{ $message }}</p>@enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="priorite" id="priorite" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('priorite') border-primary-red ring-red-200 @enderror" required>
                                ...
                                @if ($canCreateRetour && !$isAdmin)
                                    <div class="md:col-span-2">
                                        ...
                                        <textarea name="retour" id="retour" rows="3" class="mt-1 block w-full px-4 py-2 text-gray-800 rounded-lg shadow-sm border-gray-300 focus:ring-primary-red focus:border-primary-red @error('retour') border-primary-red ring-red-200 @enderror" placeholder="{{ __('Ajoutez des notes...') }}">{{ old('retour') }}</textarea>
                            ...
                            <div class="mt-8 flex items-center justify-end space-x-4">
                                <a href="{{ route('taches.index') }}" class="inline-flex items-center px-6 py-3 bg-gray-200 border border-transparent rounded-full font-bold text-sm text-gray-700 uppercase tracking-wider shadow-md btn-secondary transform hover:scale-105">
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function toggleDescriptionField() {
                    if (audioDataInput.value || (audioPlayback.src && audioPlayback.style.display !== 'none')) {
                        ...
                        if ("{{ old('description', '') }}" && !audioDataInput.value) descriptionField.value = "{{ old('description', '') }}";
                    }
                }
                ...
                function toggleAudioControls() {
                    if (descriptionField.value.trim() !== '') {
                        ...
                        if (!recordedAudioBlob && !audioDataInput.value) { playButton.disabled = true; clearButton.disabled = true; }
                    }
                }
                ...
                recordButton.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        mediaRecorder.start(); recordButton.disabled = true; stopButton.disabled = false;
                        toggleDescriptionField();
                    } catch (err) {
                        alert('Impossible d\'accéder au microphone.');
                        recordButton.disabled = false; stopButton.disabled = true; toggleDescriptionField();
                    }
                });
                ...
                if (audioDataInput.value) {
                    const parts = audioDataInput.value.split(';base64,');
                    ...
                    audioPlayback.src = URL.createObjectURL(recordedAudioBlob);
                    audioPlayback.style.display = 'block'; playButton.disabled = false; clearButton.disabled = false; recordButton.disabled = true;
                }
