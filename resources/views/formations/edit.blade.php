{{-- resources/views/formations/edit.blade.php (Updated with Checkboxes for User Assignment) --}}
<x-app-layout>
    <style>
        ...
        .checkbox-group-container { /* Renamed from radio-group-container for clarity */
        ...
        .checkbox-group-item { /* Renamed from radio-group-item for clarity */
            display: flex;
            align-items: center;
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-3xl font-bold text-gray-800 color-primary heading-bounce">
                <i class="fas fa-pencil-alt mr-3"></i> Modifier la Formation: <span class="color-primary">{{ $formation->name }}</span>
            </h2>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 card-hover-scale">
            <h3 class="text-2xl font-semibold text-gray-700 mb-6 border-b pb-3 flex items-center">
                <i class="fas fa-edit mr-3 color-primary heading-bounce"></i> Modifier les détails de la Formation
            </h3>
            <form action="{{ route('formations.update', $formation->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="lieu" {{ old('status', $formation->status) == 'lieu' ? 'selected' : '' }}>En lieu physique</option>
                            </select>
                        </div>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            ...
                            <input type="text" name="nomformateur" id="nomformateur" value="{{ old('nomformateur', $formation->nomformateur) }}" required
                    ...
                    {{-- Statut de la Formation (statut) --}}
                    <div>
                        <label for="statut" class="block text-sm font-medium text-gray-700">Statut de la Formation <span class="text-red-500">*</span></label>
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm input-group-text">
                            ...
                            <select name="statut" id="statut" required
                                ...
                                <option value="fini" {{ old('statut', $formation->statut) == 'fini' ? 'selected' : '' }}>Terminée</option>
                            </select>
                        </div>
                        @error('statut')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            ...
                            <input type="number" name="nombre_heures" id="nombre_heures" value="{{ old('nombre_heures', $formation->nombre_heures) }}" min="1" max="1000" required
                                   class="flex-1 block w-full px-3 py-2 border border-gray-300 rounded-none rounded-r-md focus:outline-none focus:ring-primary-custom focus:border-primary-custom @error('nombre_heures') border-red-500 @enderror">
                        </div>
                        @error('nombre_heures')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            ...
                            <input type="number" name="nombre_seances" id="nombre_seances" value="{{ old('nombre_seances', $formation->nombre_seances) }}" min="1" max="100" required
                                   class="flex-1 block w-full px-3 py-2 border border-gray-300 rounded-none rounded-r-md focus:outline-none focus:ring-primary-custom focus:border-primary-custom @error('nombre_seances') border-red-500 @enderror">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            ...
                            <input type="number" step="0.01" class="flex-1 block w-full px-3 py-2 border border-gray-300 rounded-none rounded-r-md shadow-sm focus:outline-none focus:ring-primary-custom focus:border-primary-custom @error('prix') border-red-500 @enderror" id="prix" name="prix" value="{{ old('prix', $formation->prix) }}" min="0" required>
                        </div>
                        @error('prix')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
...
<div class="mb-4"> {{-- Added a margin bottom for spacing --}}
    ...
    @enderror
                    ...
                    <div>
                        <label for="date" class="block text-sm font-medium text-gray-700">Date de la Formation <span class="text-red-500">*</span></label>
                        <div class="mt-1 flex rounded-md shadow-sm input-group">
                            <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm input-group-text">
                                <i class="fas fa-calendar-alt"></i>
                            </span>
                            <input type="date" class="flex-1 block w-full px-3 py-2 border border-gray-300 rounded-none rounded-r-md shadow-sm focus:outline-none focus:ring-primary-custom focus:border-primary-custom @error('date') border-red-500 @enderror" id="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($formation->date)->format('Y-m-d')) }}" required>
                        </div>
                        @error('date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                {{-- Assigné(e) - Checkboxes (allowing multiple users) --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Assigner des utilisateurs <span class="text-red-500">*</span></label>
                    <div class="checkbox-group-container"> {{-- Changed class name back to checkbox-group-container for clarity --}}
                        @php
                            // Get IDs of users currently assigned to this formation
                            $assignedUserIds = old('iduser', $formation->users->pluck('id')->toArray());
                        @endphp
                        @foreach($users as $user)
                            <div class="checkbox-group-item"> {{-- Changed class name back to checkbox-group-item for clarity --}}
                ...
                {{-- Fichier --}}
                <div>
                    <label for="file" class="block text-sm font-medium text-gray-700">Fichier (PDF, Doc, Image, Vidéo) <span class="text-gray-500 text-xs">(Max 10MB)</span></label>
                    <div class="mt-1 flex rounded-md shadow-sm input-group">
                        <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 text-sm input-group-text">
                            <i class="fas fa-paperclip"></i>
                        </span>
                        <input type="file" class="flex-1 block w-full px-3 py-2 border border-gray-300 rounded-none rounded-r-md shadow-sm focus:outline-none focus:ring-primary-custom focus:border-primary-custom @error('file') border-red-500 @enderror" id="file" name="file">
                    </div>
                    @error('file')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex justify-end space-x-4 mt-6">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-sync-alt mr-2"></i> Mettre à jour la Formation
                    </button>
                </div>
            </form>
        </div>
